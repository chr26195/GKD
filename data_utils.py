...
@torch.no_grad()
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None, test_dataset = None):
    if result is not None:
        out = result
    else:
        model.eval()
        out = model.inference(dataset, mode=args.mode)
        
    if test_dataset == None: 
        test_dataset = dataset
        test_out = out
    else:
        test_out = model.inference(test_dataset, mode=args.mode) # teacher trained/valid on full dataset, test on mask dataset
    ...
    test_acc = eval_func(
        dataset.label[split_idx['test']], test_out[split_idx['test']])
...
def load_fixed_splits(dataset, name, protocol):
    ...
    if name in ['cora', 'citeseer', 'pubmed'] and protocol == 'semi':
        ...
        for i in range(10):
            splits_file_path = '../../data/geom-gcn/splits/{}'.format(name) + '_split_0.6_0.2_'+str(i)+'.npz'
            splits = {}
...
def remove_edges(edge_index, ratio):
    E = edge_index.size(0)
    e = int(E * ratio)
    idx = torch.randperm(E)[:e]
