...
def load_nc_dataset(dataname, sub_dataname='', data_dir='../../data/'):
    ...
    if dataname == 'twitch-e':
        ...
        if sub_dataname not in ('DE', 'ENGB', 'ES', 'FR', 'PTBR', 'RU', 'TW'):
            ...
            sub_dataname = 'DE'
        dataset = load_twitch_dataset(sub_dataname, data_dir)
    elif dataname == 'fb100':
        if sub_dataname not in ('Penn94', 'Amherst41', 'Cornell5', 'Johns Hopkins55', 'Reed98'):
            ...
            sub_dataname = 'Penn94'
        dataset = load_fb100_dataset(sub_dataname, data_dir)
        ...
        dataset = load_ogb_dataset(dataname, data_dir)
    elif dataname in ('cora', 'citeseer', 'pubmed'):
        dataset = load_planetoid_dataset(dataname, data_dir)
    elif dataname in ('amazon-computer', 'amazon-photo'):
        dataset = load_planetoid_dataset(dataname, data_dir)
    elif dataname in ('coauthor-cs', 'coauthor-physics', 'corafull'):
        dataset = load_planetoid_dataset(dataname, data_dir)
    elif dataname in ('chameleon', 'cornell', 'film', 'squirrel', 'texas', 'wisconsin'):
        dataset = load_geom_gcn_dataset(dataname, data_dir)
    else:
        raise ValueError('Invalid dataname')
    return dataset


def load_twitch_dataset(lang, data_dir):
...
def load_fb100_dataset(filename, data_dir):
...
def load_ogb_dataset(name, data_dir):
...
def load_planetoid_dataset(name, data_dir):
    ...
    if name in ('cora', 'citeseer', 'pubmed'):
        ...
        torch_dataset = CoraFull(root=f'{data_dir}CoraFull', transform=transform)
...
def load_geom_gcn_dataset(name, data_dir):
